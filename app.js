...
app.get("/districts", async (req, res) => {
  ...
  try {
    ...
    const uniqueDistricts = await collection.distinct("_district", {
      _state: state,
    });
...
app.get("/cities", async (req, res) => {
  ...
  if (!state || !district)
    return res.status(400).send("State or District not specified");
  ...
  try {
    ...
    const uniqueCities = await collection.distinct("_city", {
      _state: state,
      _district: district,
    });
...
app.get("/blood-groups", async (req, res) => {
  ...
  if (!state || !district || !city)
    return res.status(400).send("State, District, or City not specified");
  ...
  try {
    ...
    const uniqueBloodGroups = await collection.distinct(
...
app.put(
  "/update-blood-group/:adminId/:bloodGroup/:action",
  async (req, res) => {
    ...
    try {
      ...
      if (bloodBank) {
        ...
        res.json({
          success: true,
          message: `Blood group ${action}ed successfully!`,
        });
      } else {
        res
          .status(404)
